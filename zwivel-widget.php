    ...
    function widget($args, $instance) {
        ...
        if( $url ) {
            if (strpos($url, "/group-direct/") !== FALSE) {
                ...
                $widget_code = '<script id="zwivelWidgetSnippet" src="https://doctor.zwivel.com/group-widget/'.$matches[1].'"></script>';
                echo $widget_code;
            } else {
                $matches = array();
                preg_match('/zwivel.com\/(.*)\/direct/', $url, $matches);
                $widget_code = '<script id="zwivelWidgetSnippet" src="https://doctor.zwivel.com/widget/'.$matches[1].'"></script>';
                echo $widget_code;
            }            
        }
    }
